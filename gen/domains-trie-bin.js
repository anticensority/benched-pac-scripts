...
function trieToIof(trie, indent) {
  ...
  if (trie['']) {
    ...
    cases = `if (ifBin(${JSON.stringify(values, null, ' ')}, dom)) { return true; }`;
...
function generateIfByHost(hosts, indent) {
  ...
  return ifUncensorByHostTrie.toString()
    .replace('__IF_BIN__', IfBin.toString())
    .replace('__SWITCH__;', trieToIof(hostsTrie, indent));
